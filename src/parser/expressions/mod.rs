use inkwell::{builder::Builder, module::Module, context::Context, types::{BasicType, BasicTypeEnum, AnyTypeEnum, BasicMetadataTypeEnum, FunctionType}};

use crate::lexer::lexer::{Token, TokenType};

pub trait ASTExpr {
    fn generate<'a>(&self, context: &'a Context, module: &Module<'a>, builder: &Builder) -> ();
...
impl ASTExpr for VoidExpr {
    fn generate(&self, context: &Context, module: &Module, builder: &Builder) -> () {
        return;
    }
    fn to_string(&self) -> String {
        "NOP".to_string()
...
// TODO: Move DataType to a separate file
#[derive(Debug, Clone)]
pub enum DataType {
    I8,
    I16,
